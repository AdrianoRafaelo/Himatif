...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('prokers', function (Blueprint $table) {
            ...
            $table->string('status')->nullable(); // status (ex: 'Perencanaan', 'Persiapan', 'Pelaksanaan', 'Selesai')
            $table->string('period')->nullable(); // periode (ex: '2024-2025')
            $table->string('report_file')->nullable(); // Path file berita acara
            $table->enum('approval_status', ['pending', 'approved', 'rejected'])->nullable(); // Status persetujuan
            $table->timestamps();
